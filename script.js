...
function renderizarProdutos() {
    ...
    const filtro =
        $(".filter.active")?.dataset.filter || "Todos";
    ...
    list.innerHTML = baixos.length
        ? baixos.map((p) => `
            <div class="alert">
                ...
                <small>
                    Atual:
                    <b>${num(p.estoque_atual)}</b>
                    ·
                    Mínimo:
                    <b>${num(p.estoque_minimo)}</b>
                </small>

                <button
                    type="button"
                    data-action="movimentacao">
                    Registrar reposição
                </button>
            </div>
        `).join("")
...
async function salvarProduto(event) {
    ...
    data.sabor =
        String(data.sabor || "").trim();

    data.estoque_inicial =
        Number(data.estoque_inicial || 0);

    data.estoque_minimo =
        Number(data.estoque_minimo || 0);

    data.unidade =
        String(data.unidade || "un").trim();
...
async function carregarResumo() {
    try {
        ...
        if ($("#s2"))
            $("#s2").textContent =
                num(data.estoque_total);

        if ($("#s3"))
            $("#s3").textContent =
                num(data.estoque_baixo);
        ...
        if ($("#s5"))
            $("#s5").textContent =
                num(data.itens_vendidos);

        if ($("#dailyRevenue"))
            $("#dailyRevenue").textContent =
                money(data.receita);
        ...
        if ($("#dailyCash"))
            $("#dailyCash").textContent =
                money(data.saldo_esperado);
...
async function prepararFechamento() {
    try {
        ...
        const esperado =
            Number(
                data.saldo_esperado || 0
            );

        if ($("#closeExpected")) {
            $("#closeExpected").textContent =
                money(esperado);

            $("#closeExpected")
                .dataset.value = esperado;
        }
...
document.addEventListener(
    ...
    async (event) => {
        ...
        if (
            event.target.closest("#addProduction")
        ) {
            await carregarReceitas();
            openModal("productionModal");
        }
