...
list.innerHTML = baixos.length
    ? baixos.map(p => `
        <div class="alert">
            ...
            <small>
                Atual: <b>${num(p.estoque_atual)}</b> ·
                Mínimo: <b>${num(p.estoque_minimo)}</b>
            </small>
            <button type="button" data-action="movimentacao">
                Registrar reposição
            </button>
        </div>
    `).join("")
...
data.sabor = String(data.sabor || "").trim();
data.estoque_inicial = Number(data.estoque_inicial || 0);
data.estoque_minimo = Number(data.estoque_minimo || 0);
data.unidade = String(data.unidade || "un").trim();
...
if (data.estoque_inicial < 0) {
    toast("O estoque inicial não pode ser negativo.", "error");
    return;
}

if (data.estoque_minimo < 0) {
    toast("O estoque mínimo não pode ser negativo.", "error");
    return;
}
...
try {
    ...
    console.error(e);
    toast(e.message || "Não foi possível cadastrar o produto.", "error");
}
```
...
if (tipo === "saída" || tipo === "saida") {
    ...
    if (
        ...
        toast(
            `Estoque insuficiente. Disponível: ${num(produto.estoque_atual)}.`,
            "error"
        );
        return;
    }
...
```
    ...
    if ($("#dailyRevenue")) {
        $("#dailyRevenue").textContent = money(data.receita);
    }

    if ($("#dailyCost")) {
        $("#dailyCost").textContent = money(data.custo);
    }

    if ($("#dailyProfit")) {
        $("#dailyProfit").textContent = money(data.lucro);
    }

    if ($("#dailyCash")) {
        $("#dailyCash").textContent = money(data.saldo_esperado);
    }
...
try {
    ...
    container.innerHTML = lista.length
        ? lista.map(c => {
            const entrada =
                String(c.tipo || "").toLowerCase() === "entrada";
        ...
        : `
            <div class="empty" style="display:block">
                <b>Nenhuma movimentação de caixa.</b>
            </div>
        `;
} catch (e) {
    console.error(e);
...
try {
    ...
    container.innerHTML = lista.length
        ? lista.map(f => `
            <div class="moveRow">
                <div>
                    ...
                    <small>
                        Saldo contado: ${money(f.saldo_contado)}
                    </small>
                </div>
        ...
        : `
            <div class="empty" style="display:block">
                <b>Nenhum fechamento registrado.</b>
            </div>
        `;
} catch (e) {
    console.error(e);
...
```
if (event.target.id === "closeCounted") {
    atualizarDiferenca();
}
```
